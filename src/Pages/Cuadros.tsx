...
import { getProductosByCategoria, type Producto } from '../api/productosApi';
...
const Cuadros = () => {
  ...
  // Lista de productos vacía - se llenará desde el API
  const [products, setProducts] = useState<Product[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Cargar cuadros del API
  useEffect(() => {
    const cargarCuadros = async () => {
      try {
        setLoading(true);
        const productosAPI = await getProductosByCategoria('cuadros');
        
        // Adaptar productos del API al formato local
        const cuadrosAdaptados = productosAPI.map((p: Producto) => ({
          id: p.id.toString(),
          name: p.nombre,
          price: p.precio,
          image: p.imagenUrl,
          description: p.descripcion,
          badge: p.categoria.nombre.toUpperCase(),
          badgeColor: obtenerColorBadge(p.nombre),
        }));
        
        setProducts(cuadrosAdaptados);
        setError(null);
      } catch (err) {
        console.error("Error al cargar cuadros:", err);
        setError("Error al cargar cuadros del API. Verifica que el servidor esté corriendo en http://localhost:8083");
      } finally {
        setLoading(false);
      }
    };
  ...
  const obtenerColorBadge = (nombre: string): string => {
    if (nombre.toLowerCase().includes('dorado') || nombre.toLowerCase().includes('premium')) return 'bg-warning';
    if (nombre.toLowerCase().includes('minimalista') || nombre.toLowerCase().includes('moderno')) return 'bg-info';
    if (nombre.toLowerCase().includes('rustico') || nombre.toLowerCase().includes('natural')) return 'bg-success';
    if (nombre.toLowerCase().includes('diploma') || nombre.toLowerCase().includes('profesional')) return 'bg-primary';
    if (nombre.toLowerCase().includes('vintage') || nombre.toLowerCase().includes('antiguo')) return 'bg-secondary';
    return 'bg-secondary';
  };
          ...
          {loading && (
            <div className="text-center my-5">
              <div className="spinner-border text-warning" role="status" style={{ width: '3rem', height: '3rem' }}>
                <span className="visually-hidden">Cargando cuadros...</span>
              </div>
              <p className="mt-3 text-muted">Cargando cuadros desde el API...</p>
            </div>
          )}
