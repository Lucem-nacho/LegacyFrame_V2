...
import { getProductos, type Producto } from "../api/productosApi";
...
const Molduras = () => {
  ...
  useEffect(() => {
    const cargarProductos = async () => {
      try {
        ...
        const productosAdaptados = productosAPI.map((p: Producto) => ({
          id: p.id.toString(),
          name: p.nombre,
          priceFrom: p.precio,
          priceTo: p.precio,
          ...
          category: p.categoria.nombre.toLowerCase() as Exclude<Category, "all">,
          badge: p.categoria.nombre.toUpperCase(),
          badgeColor: obtenerColorBadge(p.categoria.nombre),
          whatsappText: `Consulta por ${p.nombre}`,
        }));
        ...
        setError("Error al cargar productos del API. Verifica que el servidor esté corriendo en http://localhost:8083");
      } finally {
        setLoading(false);
      }
    };
  ...
  const obtenerColorBadge = (categoria: string): string => {
    const colores: Record<string, string> = {
      'grecas': 'bg-primary',
      'rusticas': 'bg-info',
      'naturales': 'bg-success',
      'nativas': 'bg-secondary',
      'finger-joint': 'bg-warning',
    };
    return colores[categoria.toLowerCase()] || 'bg-secondary';
    ...
    <div className="container my-4">
      ...
      {loading && (
        <div className="text-center my-5">
          ...
          <p className="mt-3">Cargando productos desde el API...</p>
        </div>
      )}
